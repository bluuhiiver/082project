...
- **어필리에이트별 개인 할인코드 수집**: referrals의 `affiliate_coupon`(없으면 `coupon_applied`)을 어필리에이트별로 모아 `couponByEmail`(이메일 → 콤마로 구분된 코드 목록)로 반환한다. `Index.html`이 이 값으로 `OrderMetrics.gs`의 코드별 추정 GMV와 매칭해 GMV 표에 "추정 GMV" 컬럼을 채운다 — 이 필드가 없으면 그 컬럼은 전부 `—`로만 나온다.
...
이 규칙을 바꾸고 싶으면 `Code.gs`의 `fetchUpPromoteGmv()` 안 해당 조건문만 수정하면 된다. 최신 전체 함수 코드는 이 세션 대화 기록에 남겨진 버전을 참고 — 매번 전체를 다시 붙여넣는 방식으로 갱신해왔다.
